...
const S = {
  ...
  topbar: { padding: '4px 8px', display: 'flex', alignItems: 'center' } as React.CSSProperties,
...
const App: React.FC = () => {
  ...
  return (
    <div style={S.body}>
      ...
      <div style={S.sidebar(sidebarOpen)}>
        {showSettings ? (
          ...
          <div style={{ flex: 1, display: 'flex', flexDirection: 'column', padding: 16, overflowY: 'auto' }}>
            ...
            {/* 知识管理入口 */}
            <div style={{ marginBottom: 24 }}>
              <div style={{ fontSize: 10, color: '#999', marginBottom: 8, textTransform: 'uppercase', letterSpacing: 0.5 }}>知识管理</div>
              <button onClick={() => { setShowKnowledge(true); setShowSettings(false) }} style={{
                width: '100%', padding: '10px', borderRadius: 8, border: '1px solid #e5e5e5',
                background: '#fff', cursor: 'pointer', fontSize: 13, color: '#333', textAlign: 'left'
              }}>📚 {tl('知识管理', currentLang)}</button>
            </div>
      ...
      <div style={S.main}>
        <div style={S.topbar}>
          <button onClick={() => setSidebarOpen(!sidebarOpen)} style={{ border: 'none', background: 'transparent', cursor: 'pointer', fontSize: 14, color: '#ccc', padding: '2px 4px', opacity: 0.5 }}>☰</button>
        </div>
